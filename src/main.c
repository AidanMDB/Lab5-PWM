...
void setup_tim3(void) {
    ...
    GPIOC -> MODER &= ~0x00ff000;
    // TODO: Then we set them to AF mode
    GPIOC -> MODER |=  0x00aa000;
    // TODO: Set PC6-9 to use AF0 since this corresponds to the TIM3 Ch1-4
    GPIOC -> AFR[0] &= ~0xff << (4 * 6);  // in pdf AFR7 is the seventh pin then the 4 digit combo is which alternate function to use
    GPIOC -> AFR[0] |=  0x00 << (4 * 6);  // we have AFRLow which only contains pins 0-7 each one having a 4 digit code
                                          // we are setting that 4 digit code to 0000 for AFR0 then 4digits*6pins over = 24 pins moved
    GPIOC -> AFR[1] &= ~0xff; //not using the fancy calculation here cause im just setting pins 8 and 9
    GPIOC -> AFR[1] |=  0x00;
    // AFR[0] -> AFRL
    // AFR[1] -> AFRH
    ...
    //TIM3 -> CCMR1 &= ~TIM_CCMR1_OC1M_0; // turn of the 0th bit
    TIM3 -> CCMR1 |=  TIM_CCMR1_OC1M_2 | TIM_CCMR1_OC1M_1; // CCMR1 is channels 1 & 2
    //TIM3 -> CCMR1 &= ~TIM_CCMR1_OC2M_0;
    TIM3 -> CCMR1 |=  TIM_CCMR1_OC2M_2 | TIM_CCMR1_OC2M_1; // OCxM is which channel to set to output
    //TIM3 -> CCMR1 &= ~TIM_CCMR2_OC3M_0;
    TIM3 -> CCMR2 |=  TIM_CCMR2_OC3M_2 | TIM_CCMR2_OC3M_1; // CCMR2 is channels 3 & 4
    //TIM3 -> CCMR1 &= ~TIM_CCMR2_OC4M_0;
...
void setup_tim1(void) {
    ...
    TIM1 -> CCMR1 |= TIM_CCMR1_OC1M_2 | TIM_CCMR1_OC1M_1; // CCMR1 is channels 1 & 2
    TIM1 -> CCMR1 |= TIM_CCMR1_OC2M_2 | TIM_CCMR1_OC2M_1; // OCxM is which channel to set to output
    TIM1 -> CCMR2 |= TIM_CCMR2_OC3M_2 | TIM_CCMR2_OC3M_1; // CCMR2 is channels 3 & 4
...
void enable_ports(void) {
    ...
    GPIOC -> OTYPER |=  0x000000f0;RCC -> AHBENR |= RCC_AHBENR_GPIOBEN;
    GPIOB -> MODER &= ~0x003fffff;
    GPIOB -> MODER |=  0x00155555;
